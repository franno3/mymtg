---
tags: [karta, stwór, zielony]
kolor: ["G"]
typ: Creature
podtyp: Human Warrior
edycja: "[[Edycje/MOC|March of the Machine Commander (MOC)]]"
koszt_many: "{1}{G}{G}"
rzadkość: Rare
cmc: 3
color_identity: G
commander_legal: true
papierowe_mam: 1                    # liczba posiadanych egzemplarzy
papierowe_potrzebuje: 0             # liczba potrzebnych egzemplarzy
papierowe_status: "Mam"
elektroniczne_mam: 0
elektroniczne_potrzebuje: 0
elektroniczne_status: "Nie mam"     # Mam | Nie mam | Szukam
---

# Champion of Lambholt

![Champion of Lambholt](https://cards.scryfall.io/normal/front/4/6/46eff31d-f460-48f2-aab7-8b9b89cd87fe.jpg)

> [!info] Statystyki
> **Koszt many:** `{1}{G}{G}`
> **Typ:** Creature — Human Warrior
> **Siła/Wytrzymałość:** 1/1
> **Rzadkość:** Rare

---

## Tekst karty

### Angielski (oryginał)
> Creatures with power less than this creature's power can't block creatures you control.
> Whenever another creature you control enters, put a +1/+1 counter on this creature.

### Polski (tłumaczenie)
> Stworzenia o sile mniejszej niż siła tego stworzenia nie mogą blokować stworzeń, które kontrolujesz.
> Ilekroć inne stworzenie, które kontrolujesz, wchodzi na pole bitwy, umieść żeton +1/+1 na tym stworzeniu.

---

## Użycie w taliach
- [[Decklists/Token Triumph — Commander|Token Triumph — Commander]]

---

## Posiadanie

> [!check] Status kolekcji

| | Posiadam | Potrzebuję | Status | Uwagi |
|---|---|---|---|---|
| **Papierowe** | 1 | 0 | Mam | |
| **Elektroniczne** | 0 | 0 | Nie mam | |
